...
无论是前面的 ReAct 还是后面的 Reflection，都有一个共同的短板，就是缺乏对于任务的大局观。

当提出一个问题的时候，要先去规划，再去执行，规划就是把一个任务拆成每个小的任务，每个小的任务再使用 ReAct 或者是 Reflection 模式执行。

在工程实现上，一个标准的 Plan & Execute 架构通常包含三个核心组件，我们称之为“铁三角”：

1. Planner（规划器）：大脑的“前额叶”
   职责：接收用户的复杂目标，将其拆解为一系列原子化、可执行的子任务（Sub-tasks）。
   输出格式：通常是一个列表（List）或有向无环图（DAG, Directed Acyclic Graph）。每个节点包含：Task_ID, Description, Dependencies（依赖的前置任务）, Tool_Required。
2. Executor（执行器）：不知疲倦的“手脚”
   职责：读取 Planner 生成的计划，按顺序（或根据依赖关系并发）执行每一个子任务。
   机制：它本质上是一个受控的 ReAct 循环。但它的 Thought 被严格限制在“如何完成当前这个特定的子任务”，而不是漫无目的地发散。
3. Re-planner / Monitor（重规划器/监控器）：动态纠偏的“监理”
   职责：在执行过程中，如果 Executor 报告“任务失败”（如 API 报错、找不到结果），或者用户中途修改了需求，Re-planner 会介入。
   机制：它不会从头开始，而是基于当前的执行状态（State），只修改计划中受影响的部分（例如：“步骤 2 失败，替换为备选方案 B”），然后让 Executor 继续。

![1781529345153](images/01_搭建简单的Agent/1781529345153.png)
...
因为在真实工作中，单体Agent无法满足复杂场景，例如：需要开发一个网站，需要有前端、后端、数据库专家、项目经理、测试人员；当使用单体 Agent 完成不同的工作时，会出现幻觉，因为不同的工作需要不同的上下文和Prompt，若单体 Agent 在完成前端工作之后，立马投入到后端的开发中，就会出现注意力稀释和Prompt冲突，相当于把前端已经完成的内容，又作为 Context 喂到了后端的工作中；单体 Agent 会对自己完成的工作十分自信，也由此会对输出的结果产生幻觉。

![1782822990914](image/01_搭建简单的Agent/1782822990914.png)

### 6.2. MultiAgent 代码

#### 6.2.1. 使用 Tool Call 实现

流程图

![1782824071289](image/01_搭建简单的Agent/1782824071289.png)
...
def main() -> None:
    ...
    file_agent = create_react_agent(
        llm,
        tools=[list_files, move_file],
        state_modifier=FILE_AGENT_PROMPT,
        name="file_agent",
    )
    code_agent = create_react_agent(
        llm,
        tools=[write_file],
        state_modifier=CODE_AGENT_PROMPT,
        name="code_agent",
    )
...
![1782825008212](image/01_搭建简单的Agent/1782825008212.png)
